...
class HypertableStub(apiproxy_stub.APIProxyStub):
  ...
  def _Create_Obj_Datastore(self, kind):
    ...
    try:
      ...
      self.__client.hql('create table \'%s\' (meta, entity)' %table_name)
  ...
  def _Dynamic_Put(self, put_request, put_response):
    ...
    for entity in entities:
      ...
      self._Create_Obj_Datastore(kind)
    ...
    for kind in kind_cells_dict.keys():
      ...
      for entity in entities:
        ...
        mutator.set(encoded_key, 'meta', 'kind', kind)
        mutator.set(encoded_key, 'meta', 'app', self.__app_id)
  ...
  def _Dynamic_Drop(self, drop_request, drop_response):
    ...
    table_name = str('%s_%s' % (self.__app_id, kind))
    try:
      self.__client.hql('drop table \'%s\'' %table_name)
    except RuntimeError:
      logging.warning('drop_table: No table named %s' %table_name)
